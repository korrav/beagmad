...
public:
	...
	inline unsigned int get_amount(void);
	...
	inline void get_gain(short* g);
	inline void set_freq(const int& freq);
	inline unsigned int get_freq(void);
	DataADC();
	virtual ~DataADC();
};
...
unsigned int DataADC::get_amount(void) {
...
inline void* DataADC::get_data(void) {
...
inline void DataADC::get_gain(short * g) {
	for (short& x : gain_)
		*(g++) = x;
	return;
}
...
inline unsigned int DataADC::get_freq(void) {
	return freq_;
}
